...
// axios.defaults.baseURL = 'https://trafikceza.ibb.gov.tr/api/api'
    ...
    /* local: when on development, live: when production */
    axios.defaults.baseURL = getAPIBaseUrl('local');

    function getAPIBaseUrl(server = 'live') {
        if (server === 'live') {
            return 'https://oguzhansenyigit.com/api/api/';
        }
