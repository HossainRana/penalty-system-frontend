...
// import { setFilteredTollData } from './../../../store/reducers/toll/toll.actions';
...
export default function Dashboard(props) {
  ...
  const [directorateData,setDirectoreData]=useState({
    month: 0,
    year: 0,
  })
...
const filterPlate=async()=>{
  console.log('plate data is ',plateData);
  const response =await axios.get("license-penalty?status=" + status+"&year="+plateData.year+"&month="+plateData.month);
  const result =response.data;
  console.log(result);
}

const filterDirectorate=()=>{
  console.log('directorate data is ',directorateData);
}
const filterDepartment=()=>{
  console.log("department data is ",departmentData);
}
const filterSecretary=()=>{
  console.log("secretary data is ",secretaryData);
}
  ...
  useEffect(() => {
    const getFinesByLicensePlate = async () => {
      ...
      let result = await axios.get("license-penalty?status=" + status+"&year="+plateData.year+"&month="+plateData.month );
      setLicensePenaltyList(result.data);
      setFilteredLicensePenaltyList(result.data);
    };

    getFinesByLicensePlate();
  }, [status,plateData.year,plateData.month]);
  ...
  useEffect(() => {
    const getDirectoryFineList = async () => {
      ...
      let result = await axios.get("directorate-fines?status=" + status2 +"&year="+directorateData.year+"&month="+directorateData.month );
      setdirectoryPenaltyList(result.data);
      setfilteredDirectoratePenaltyList(result.data);
    };

    getDirectoryFineList();
  }, [status2,directorateData.year,directorateData.month]);
  ...
  useEffect(() => {
    const getDepartmentFineList = async () => {
      ...
      let result = await axios.get("department-heads?status=" + status3+"&year="+departmentData.year+"&month="+departmentData.month);
      setdepartmentPenaltyList(result.data);
      setfilteredDepartmentPenaltyList(result.data);
    };

    getDepartmentFineList();
  }, [status3,departmentData.year,departmentData.month]);
  ...
  useEffect(() => {
    const getGeneralFineList = async () => {
      ...
      let result = await axios.get("general-secretariats?status=" + status4+"&year="+secretaryData.year+"&month="+secretaryData.month);
      setsecretariatsPenaltyList(result.data);
      setfilteredSecretariatsPenaltyList(result.data);
    };

    getGeneralFineList();
  }, [status4,secretaryData.year,secretaryData.month]);
  ...
  const filteredDirectorateList = (searchText) => {
    const filteredList = directoryPenaltyList.filter((item) => {
      if (searchText == "") {
        ...
        item.unit.toLocaleLowerCase().includes(searchText.toLocaleLowerCase())
      ) {
        return item;
      }
    });
      ...
      for (let department of item.departments) {
        if (
          ...
          let index = filteredList.findIndex(
            (listitem) => listitem.boss === department.boss
          );
          if (index > -1) {
  ...
  const getLicensePenaltyData = () => {
    let columns = [
      ...
      { title: "CEZA SAHİBİ", width: { wpx: 280 }, style: exportCommonStyle },
      { title: "ADET", width: { wpx: 140 }, style: exportCommonStyle },
      {
        title: "TRAFİK CEZALARI",
        width: { wpx: 180 },
        style: exportCommonStyle,
      },
    ];
  ...
  const getDepartmentPenaltyData = () => {
    ...
    const data = [];
    const exportData = selectedDepartmentPenalties.length
      ? selectedDepartmentPenalties
  ...
  const ExportDirectoratePenalty = () => {
    ...
    return (
      <ExportExcelFile
        ...
        fileName={"müdürlük_cezalar_report"}
      />
    );
  };
  ...
  const ExportDepartmentPenalty = () => {
    ...
    return (
      <ExportExcelFile
        ...
        fileName={"bölüm_başkanları_rport"}
      />
    );
  };
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Paper style={{ margin: "15px 0" }}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={4}>
                    <TextField
                      id="searchOne"
                      onChange={(event) => {
                        filterLicensePenalties(event.target.value);
                      }}
                      label="Plaka Cezalarını Arayın"
                      variant="outlined"
                      style={{ width: "100%" }}
                    />
                  </Grid>
                ...
                <Grid container spacing={2} columnGap={10}>
                  ...
                  <Table stickyHeader aria-label="Plaka Ceza Tablosu">
                    ...
                    <TableBody>
                      {filteredLicensePenaltyList &&
                        ...
                        filteredLicensePenaltyList.map((item, index) => (
                          <TableRow
                            key={index}
                            sx={{
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={4}>
                    <TextField
                      ...
                      label="Müdürlük Cezalarını Arayın"
                      variant="outlined"
                      style={{ width: "100%" }}
                    />
                  </Grid>
                        ...
                        filteredDirectoratePenaltyList
                          ...
                          .map((item, index) => (
                            ...
                            >
                              ...
                              <TableCell align="right">
                                {Number(item.amount || 0).toLocaleString()} TL
                              </TableCell>
                            </TableRow>
                          ))}
                      {(!filteredDirectoratePenaltyList ||
                    ...
                    <TableHead>
                      <TableRow>
                        ...
                        >
                          ...
                          <Tooltip title="Clear Selection">
                            <IconButton
                              onClick={() => setSelectedDepartmentPenalties([])}
                              style={{ color: "#ff0000", cursor: "pointer" }}
                            >
                              <Refresh />
                            </IconButton>
                                ...
                                <FormControlLabel
                                  control={
                                    <Checkbox
                                      ...
                                      onChange={(e) =>
                                        toggleDepartmentPenaltySelection(item)
                                      }
                                    />
                                  }
                                />
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={4}>
                    <TextField
                      id="searchFour"
                      onChange={(event) => {
                        filtereSecretariatsList(event.target.value);
                      }}
                      label="Search"
                      variant="outlined"
                      style={{ width: "100%" }}
                                        ...
                                        <Checkbox
                                          ...
                                          checked={selectedSecretariatsPenalties.some(
                                            (selItem) =>
                                              ...
                                              selItem.total_count ===
                                                dept.total_count &&
                                              selItem.amount === dept.amount
                                          )}
                                          onChange={(e) =>
                                            toggleSecretariatsPenaltySelection(
                                              dept
                                            )
                                          }
                                        />
